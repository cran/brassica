...
\newcommand{\ib}[1]{\texttt{#1}}
\newcommand{\iib}[1]{\hspace*{1em}\texttt{#1}}
...
Whitespace is ignored; be wary of \ib{F OR} and \ib{X OR}.
...
The unary identity operator, \ib{+}, acts only on the value to its immediate right.
...
\iib{DEF FNA(X) = X + C}\\
\iib{DEF fnb\$(A\$,n) = MID\$(A\$,n,1)}\\
\iib{PRINT FNA(7) fnb\$("ABC",2)}
}\yspace
...
sign).
(Use \ib{STR\$(X)} to suppress these.)
The width of the terminal is divided into `print zones' of 14 spaces each.
Consider;\yspace\\
\iib{PRINT A;BTAB(16)CHR\$(34)SPC(4)":",C D\$;}\yspace\\
...
Use \ib{X = 1} for a new variate, or \ib{X = 0} for the previous one.
}
...
the right margin, if \ib{X} is negative). Can only be used within \ib{PRINT}
statements.}
\bit{TAN(X)}{Tangent of \ib{X}.}
\bit{TTW(1)}{Width of the terminal, in characters.}
\bit{VAL(X\$)}{Converts \ib{X\$} to its numerical value.}\yspace\\

The dummy argument to \ib{POS}, \ib{SYST}, and \ib{TTW} must have the value
\ib{1}.
...
\mat{A MOD B}{A-INT(A/B)*B}
\mat{LOG10(X)}{LOG(X)/LOG(10)}
\mat{SEC(X)}{1/COS(X)}
\mat{CSC(X)}{1/SIN(X)}
\mat{COT(X)}{1/TAN(X)}
